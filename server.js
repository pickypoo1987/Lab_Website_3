...
app.get('/player_info/select_player', function(req, res) {
  var p_id = req.query.player_choice;
  var query1= 'select id, name from football_players;';
  var query2 = 'select * from football_players where id=' + p_id + ';';
  var query3 = 'select count(*) from football_games where ' + p_id + ' = any(players);';
  db.task('get-everything', task => {
      return task.batch([
          task.any(query1),
          task.any(query2),
